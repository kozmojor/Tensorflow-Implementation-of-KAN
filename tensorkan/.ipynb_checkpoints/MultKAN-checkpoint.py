...
class MultKAN(tf.keras.Model):
    ...
    def fix_symbolic(self, l, i, j, fun_name, fit_params_bool=True, a_range=(-10, 10), b_range=(-10, 10), verbose=True,
        ...
        if not fit_params_bool:
            ...
            r2 = self.symbolic_fun[l].fix_symbolic(i, j,
                                                   fun_name, x, y, a_range=a_range, b_range=b_range, verbose=verbose)
            if mask[i, j] == 0:
                r2 = - 1e8
    ...
    def fit(self, dataset, opt="Adam", steps=100, log=1, lamb=0., lamb_l1=1., lamb_entropy=2., lamb_coef=0., lamb_coefdiff=0., update_grid=True, grid_update_num=10, loss_fn=None, lr=1., start_grid_update_step=-1, stop_grid_update_step=50, batch=-1,
        ...
        #     else:
        #         reg_ = tf.constant(0.0, dtype=tf.float32)
    ...
    #
    #     if loss_fn == None:
    #         loss_fn = loss_fn_eval = lambda x, y: tf.reduce_mean(tf.square(x - y))
    #     else:
    ...
    #         global train_loss, reg_
    #         optimizer.zero_grad()
    #         pred = self.call(dataset['train_input'][train_id], singularity_avoiding=singularity_avoiding, y_th=y_th)
    #         train_loss = loss_fn(pred, dataset['train_label'][train_id])
    #         if self.save_act:
    #             if reg_metric == 'edge_backward':
    #                 self.attribute()
    ...
    #             self.save_act = True
    #
    #         train_indices = tf.random.shuffle(tf.range(tf.shape(dataset['train_input'])[0]))[:batch_size]
    #         test_indices = tf.random.shuffle(tf.range(tf.shape(dataset['test_input'])[0]))[:batch_size_test]
    #
    #         train_input_batch = tf.gather(dataset['train_input'], train_indices)
    #         train_label_batch = tf.gather(dataset['train_label'], train_indices)
    #         test_input_batch = tf.gather(dataset['test_input'], test_indices)
    #         test_label_batch = tf.gather(dataset['test_label'], test_indices)
    #
    #         if _ % grid_update_freq == 0 and _ < stop_grid_update_step and update_grid and _ >= start_grid_update_step:
    #             self.update_grid(train_input_batch)
    #
    #         if opt == "LBFGS":
    #             optimizer.step(closure)
    #
    #         if opt == "Adam":
    #             pred = self.forward(train_input_batch, singularity_avoiding=singularity_avoiding, y_th=y_th)
    #             train_loss = loss_fn(pred, train_label_batch)
    #             if self.save_act:
    #                 if reg_metric == 'edge_backward':
    #                     self.attribute()
    #                 if reg_metric == 'node_backward':
    ...
    #             optimizer.step()
    #             with tf.GradientTape() as tape:
    #                 predictions = self.call(x_train, singularity_avoiding=singularity_avoiding, y_th=y_th)
    ...
    #                     reg_ = 0.0
    #
    #                 total_loss = train_loss + lamb * reg_
    #                 # 计算总损失并优化
    #                 loss = train_loss + lamb * reg_
    #                 with tf.GradientTape() as tape:
    #                     pred = self.call(train_input_batch, singularity_avoiding=singularity_avoiding, y_th=y_th)
    #                     loss = loss_fn(pred, train_label_batch) + lamb * reg_
    #
    #                 gradients = tape.gradient(loss, self.trainable_variables)
    #                 optimizer.apply_gradients(zip(gradients, self.trainable_variables))
    #
    #             # 计算测试集上的损失
    #             test_loss = loss_fn_eval(self.call(test_input_batch), test_label_batch)
    #
    #             # 如果有其他指标，进行记录
    ...
    #                         (tf.sqrt(train_loss).numpy(), tf.sqrt(test_loss).numpy(), reg_.numpy())
    #                     )
    #                 else:
    #                     string = ''
    #                     data = ()
    ...
    #             # 保存训练图像
    #             if save_fig and _ % save_fig_freq == 0:
    #                 self.plot(folder=img_folder, in_vars=in_vars, out_vars=out_vars, title="Step {}".format(_),
    #                           beta=beta)
    ...
    def prune_node(self, threshold=1e-2, mode="auto", active_neurons_id=None, log_history=True):
        ...
        for i in range(len(self.acts_scale)):
            ...
            if i < len(self.acts_scale) - 1:
                ...
                model2.node_bias[i].assign(tf.gather(model2.node_bias[i], active_neurons_up[i + 1]))
                model2.node_scale[i].assign(tf.gather(model2.node_scale[i], active_neurons_up[i + 1]))
                model2.subnode_bias[i].assign(tf.gather(model2.subnode_bias[i], active_neurons_down[i]))
                model2.subnode_scale[i].assign(tf.gather(model2.subnode_scale[i], active_neurons_down[i]))
    ...
    def evaluate(self, dataset):
        ...
        evaluation['test_loss'] = torch.sqrt(
            torch.mean((self.call(dataset['test_input']) - dataset['test_label']) ** 2)).item()
